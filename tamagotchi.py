...
def GameStart():
    class Tpet():
        ...
        def play(self):
            ...
            else: print(pet.name, ' Не хочет играть.')
    ...
    while (pet.game == True) and ((pet.weight > 0) and (pet.thirst > 0) and(pet.happiness > 0)and (pet.years < 13)) :
        age = pet.years + 0.05
        ...
        print('-----------------------------------')
        print('|  ', round(pet.years,2),'| ', round(pet.weight,2),' | ', round(pet.thirst,2),' | ', round(pet.happiness,2),' |')
        print('-----------------------------------')
        print('')
